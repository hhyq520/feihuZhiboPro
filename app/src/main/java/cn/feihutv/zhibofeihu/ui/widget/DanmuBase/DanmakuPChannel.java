 ...
 */
public class DanmakuPChannel extends FrameLayout {
    ...
    public DanmakuPChannel(Context context) {
        super(context);
        init();
    }


    public DanmakuPChannel(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public DanmakuPChannel(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public DanmakuPChannel(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public void mStartAnimation(DanmuModel entity) {
        ...
        if (mEntity != null) {
            final View view = View.inflate(getContext(), R.layout.item_pc_live_danmu, null);
                    ...
                    public void onAnimationEnd(Animator animator) {
                        if (!((Activity) getContext()).isDestroyed()) {//防止内存溢出
                            new Handler().post(new Runnable() {
                                public void run() {
                                    view.clearAnimation();
                                    DanmakuPChannel.this.removeView(view);
                                    if (danAction != null) {
                                        danAction.pollDanmu();
                                    }
                                }
