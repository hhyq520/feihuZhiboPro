...
import cn.feihutv.zhibofeihu.utils.glideutil.CustomOptions;
...
public class PlayShakeDialog extends Dialog {
...
//                        linClock.setVisibility(View.VISIBLE);
    ...
    public PlayShakeDialog(Context context, GetGameStatusResponce.GameStatusData data, String gameResultJson, int jcissueRound) {
        ...
        this.getIssueRound = jcissueRound;
    }
            ...
            if (getIssueRound == issueRound) {
                try {
                    ...
                    if (left < 20 && left > 17) {
                        ...
                        Glide.with(mContext).load(filmList.get(result.get(0)))
                                .apply(CustomOptions.getOptions(R.drawable.face, DiskCacheStrategy.ALL)).transition(DrawableTransitionOptions.withCrossFade(1000)).into(headImg1);
                        if (timer2 == null) {
                            timer2 = new Timer();
                            timer2.schedule(task2, 100, 100);
                        }
                        ...
                        GlideApp.loadImg(mContext, filmList.get(result.get(0)), R.drawable.face, DiskCacheStrategy.RESOURCE, 1000, headImg1);
                        GlideApp.loadImg(mContext, filmList.get(result.get(1)), R.drawable.face, DiskCacheStrategy.RESOURCE, 1000, headImg2);
                        if (timer2 == null) {
                            timer2 = new Timer();
                            timer2.schedule(task2, 100, 100);
                        }
                        ...
                        GlideApp.loadImg(mContext,
                                filmList.get(result.get(0)),
                                R.drawable.face,
                                DiskCacheStrategy.RESOURCE,
                                1000,
                                headImg1);
                        GlideApp.loadImg(mContext,
                                filmList.get(result.get(1)),
                                R.drawable.face,
                                DiskCacheStrategy.RESOURCE,
                                1000,
                                headImg2);
                        GlideApp.loadImg(mContext,
                                filmList.get(result.get(2)),
                                R.drawable.face,
                                DiskCacheStrategy.RESOURCE,
                                1000,
                                headImg3);
