...
public class ZQImageViewRoundOval extends ImageView {
    ...
    public static final int TYPE_OVAL_ROUND = 3;//椭圆角矩形
    ...
    protected void onDraw(Canvas canvas) {
        ...
        } else if (mType == TYPE_OVAL_ROUND) {
            mPaint.setColor(Color.RED);
            canvas.drawRoundRect(mRect, mRoundRadius*3, mRoundRadius/2, mPaint);
        }
    }
    ...
    private void setBitmapShader() {
        ...
        } else if (mType == TYPE_ROUND || mType == TYPE_OVAL || mType == TYPE_OVAL_ROUND) {
            // 如果图片的宽或者高与view的宽高不匹配，计算出需要缩放的比例；缩放后的图片的宽高，一定要大于我们view的宽高；所以我们这里取大值；
            scale = Math.max(getWidth() * 1.0f / bitmap.getWidth(), getHeight() * 1.0f / bitmap.getHeight());
            if(mType == TYPE_OVAL_ROUND){
            	mRoundRadius = getWidth()/4;
            }
        }
