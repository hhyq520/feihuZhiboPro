...
public class CustomSqliteActor extends SQLiteActor {
    ...
    private String ZBICON = "zbicon";
    private String ZBNAME = "zbname";
    private String DOWNSTATE = "downState";
    ...
    public ContentValues onCreate(@NotNull RealMission mission) {
        ...
        if (mission.getActual() instanceof CustomMission) {
            ...
            cv.put(TITLE, customMission.getTitle());
            cv.put(ZBICON, customMission.getZbIcon());
            cv.put(ZBNAME, customMission.getZbName());
            cv.put(DOWNSTATE, -1);
        }
        return cv;
    }
    ...
    public Mission onGetAllMission(@NotNull Cursor cursor) {
        ...
        String zbIcon = cursor.getString(cursor.getColumnIndexOrThrow(ZBICON));
        if (zbIcon == null || zbIcon.isEmpty()) {
            zbIcon = "no icon";
        }
        String zbName = cursor.getString(cursor.getColumnIndexOrThrow(ZBNAME));
        if (zbName == null || zbName.isEmpty()) {
            zbName = "no name";
        }

        int state = cursor.getInt(cursor.getColumnIndexOrThrow(DOWNSTATE));
        return new CustomMission(mission, img,title,zbName,zbIcon,state);
    }
}
